...
static void __kmp_alloc_task_q(kmp_info_t *thread,
			       kmp_thread_data_t *thread_data, kmp_task_team_t *task_team);
#endif
...
static kmp_int32 __kmp_push_task(kmp_int32 gtid, kmp_task_t *task) {
  ...
  kmp_uint64 last_q = thread_data->td.last_q;
    ...
    bool got_steal_req = false;
    kmp_uint64 steal_req_id;
    //if (last_q > 0) {
...
#ifdef KMP_USE_LL_WORKSTEALING
  if (got_steal_req) {
    kmp_uint64 stealer_id = steal_req_id >> 40;
    target_tid = stealer_id;
    last_q = (target_tid < gtid) ? task_team->tt.tt_nproc + target_tid - gtid :
	      target_tid - gtid; //abs((kmp_int64)target_tid - (kmp_int64)gtid);
  } else {
#endif
    target_tid = gtid + last_q;
  ...
  if (thread_data->td.td_task_q == NULL) {
    __kmp_alloc_task_q(thread, thread_data, task_team);
  ...
  while (target_thread_data->td.td_task_q[last_q]->td_deque[target_thread_data->td.td_task_q[last_q]->td_deque_head] 
	...
	double ts = tv.tv_sec * 1E9 + tv.tv_nsec;
	KA_TRACE(1,
		 ("__kmp_push_task: T#%d PUSH FAILED TO T#%dQ#%d, got_steal_req %d, %p %f\n", gtid, target_tid, last_q, got_steal_req, taskdata, ts ));
#endif
	return TASK_NOT_PUSHED;
#ifndef KMP_USE_XQUEUE
      }
  ...
  // Wrap index.
  task_q->td_deque_head = (task_q->td_deque_head + 1) & TASK_DEQUE_MASK(target_thread_data->td);

  target_thread_data->td.last_q_accessed = last_q;
  //if (last_q == 0)
  //  thread_data->td.has_items[0] = 1;
  //Update the corresponding bit of this thread's q_status to 1 since we pushed a task. 
  //This is just an approximate status since a deque could be happening at the same time.
  //However it is not an issue since the consequence is to check this queue and move on to another..
  //thread_data->td.q_status | (kmp_uint64)pow(2, last_q);

  KA_TRACE(10, ("__kmp_push_task: T#%d returning TASK_SUCCESSFULLY_PUSHED to T#%d: "
  ...
  if (got_steal_req) {
    ...
    KA_TRACE(1,
	     ("__kmp_push_task: T#%d PUSH SUCCESS %p %f tasktype: %d\n", gtid, taskdata, ts, taskdata->td_flags.tasktype));
  }
#endif  
  ...
  if (flags->proxy == TASK_PROXY || flags->detachable == TASK_DETACHABLE) {
    ...
    if ((thread->th.th_task_team) == NULL) {
	 ...
	 setup a task team and propagate it to the thread */
      ...
      //thread->th.th_task_team->tt.tt_threads_data[gtid].td.stealing_enabled = TRUE;
#endif
    }
    kmp_task_team_t *task_team = thread->th.th_task_team;
...
#ifdef KMP_USE_XQUEUE                                                                               
      if (thread_data->td.td_task_q == NULL) {
        __kmp_alloc_task_q(thread, thread_data, thread->th.th_task_team); 
      }
#else                                                                                               
      // No lock needed since only owner can allocate                                                   
      if (thread_data->td.td_deque == NULL) {                                                           
...
static kmp_int32 __kmpc_omp_taskwait_template(ident_t *loc_ref, kmp_int32 gtid,
  ...
  if (__kmp_tasking_mode != tskm_immediate_exec) {
	...
	KA_TRACE(5, ("__kmpc_omp_taskwait: T#%d task %p still waiting %d\n",
		     gtid, taskdata, taskdata->td_incomplete_child_tasks.load()));
...
#ifdef KMP_USE_XQUEUE
static kmp_task_t *__kmp_remove_aux_task(kmp_info_t *thread, kmp_int32 gtid,
					 kmp_task_team_t *task_team,
					 kmp_int32 is_constrained, kmp_uint64 *last_qid) {
  kmp_task_t *task;
  ...
  kmp_uint64 nz_idx = 0; //for optimizing work stealing
  bool got_steal_req = false;
  ...
  if (thread_data->td.last_q_accessed > 0) {
	...
	*last_qid = thread_data->td.last_q_accessed;// & (thread_data->td.num_queues - 1);
	KA_TRACE(10, ("__kmp_remove_aux_task(exit #1): T#%d:Q#%d %p removed: "
		      "tail=%u\n",
  ...
  if ((taskdata == NULL) || (got_steal_req && nz_idx == 0)) {
  ...
  if (got_steal_req)
  {
	...
	if(TCR_4(task_q->td_deque[task_q->td_deque_tail]) != NULL) {
	  ...
	  kmp_uint64 stealer_id = steal_req_id >> 40;
	  if (stealer_id == thread_data->td.steal_req_id >> 40) {
	    //Steal request can get overwritten, but asker is still waiting at this point
	    //Since round has not been incremented, so we can still steal a task.
	    kmp_thread_data_t *stealer_data = &task_team->tt.tt_threads_data[stealer_id];
	    kmp_taskdata_t *stolen_task = (kmp_taskdata_t *)task_q->td_deque[task_q->td_deque_tail];

	    //I can push here since I am the only enqueuer to the target queue
	    ...
	    kmp_taskq_t *stealer_task_q = stealer_data->td.td_task_q[last_q];
	    if (TCR_4(stealer_task_q->td_deque[stealer_task_q->td_deque_head]) == NULL) {
	      stealer_task_q->td_deque[stealer_task_q->td_deque_head] = stolen_task;
	      // Wrap index.
	      stealer_task_q->td_deque_head = (stealer_task_q->td_deque_head + 1) &
		TASK_DEQUE_MASK(stealer_data->td);

	      stealer_data->td.last_q_accessed = last_q;
  ...
  ts = tv.tv_sec * 1E9 + tv.tv_nsec;
  KA_TRACE(1,
	   ("__kmp_remove_aux_task: T#%d POP AUX SUCCESS %p %f tasktype: %d\n", gtid, taskdata, ts, taskdata->td_flags.tasktype));
#endif
  task = KMP_TASKDATA_TO_TASK(taskdata);
  return task;
  ...
  if (round == thread_data->td.round)
	 ...
	 != NULL)
	{
	  ...
	  if (stealer_id == thread_data->td.steal_req_id >> 40) {
	    ...
	    kmp_uint64 last_q = (stealer_id < gtid) ? task_team->tt.tt_nproc + stealer_id - gtid :
	      stealer_id - gtid; //abs((kmp_int64)stealer_id - (kmp_int64)gtid);
	    kmp_taskq_t *task_q = stealer_data->td.td_task_q[last_q];
	    if (TCR_4(task_q->td_deque[task_q->td_deque_head]) == NULL) {
	      task_q->td_deque[task_q->td_deque_head] = stolen_task;
	      // Wrap index.
	      task_q->td_deque_head = (task_q->td_deque_head + 1) & TASK_DEQUE_MASK(stealer_data->td);

	      stealer_data->td.last_q_accessed = last_q;

	      thread_data->td.td_task_q[0]->td_deque[thread_data->td.td_task_q[0]->td_deque_tail] = NULL;
	      thread_data->td.td_task_q[0]->td_deque_tail = 
		(thread_data->td.td_task_q[0]->td_deque_tail + 1) & TASK_DEQUE_MASK(thread_data->td);
	      //}
#ifdef XQUEUE_TRACE		
	      clock_gettime(CLOCK_REALTIME, &tv);
	      double ts = tv.tv_sec * 1E9 + tv.tv_nsec;
...
static kmp_task_t *__kmp_steal_task(kmp_info_t *victim_thr, kmp_int32 gtid,
  ...
  if (ntasks == 0) {
    ...
    ts = tv.tv_sec * 1E9 + tv.tv_nsec;
    KA_TRACE(1,
	     ("__kmp_steal_task: T#%d NO TASKS TO SERVE %f\n", gtid, taskdata, ts));
#endif
    return NULL;
  }
  ...
  //Do not steal from self. We check this before coming here, so not needed again.
  if (gtid == victim_tid) return NULL;

  kmp_thread_data_t *thread_data = &threads_data[gtid];
  kmp_uint64 num_tries = 0;
  if ((victim_td->td.steal_req_id  & (kmp_uint64)((1ULL << 40) - 1ULL)) < victim_td->td.round)
    {
      //Get a local copy of the victim's round.
      //It could get incremented after reading this!!
	...
	kmp_uint64 self_query = self_r + ((((unsigned long long)gtid) << 40));
  ...
  //5000000000
	if (++num_tries > 1000000 && thread_data->td.stolen_task == nullptr) {
	  ...
	  double ts = tv.tv_sec * 1E9 + tv.tv_nsec;
	  KA_TRACE(1,
		   ("__kmp_steal_task: T#%d STEAL TASK INVALIDATED %f %lu\n", gtid, ts, num_tries));
#endif
	  break;
	}
      }
...
// sUSE_ITT_BUILD_ARG(void *itt_sync_obj)pinner is the location on which to spin.
  ...
  while (1) { // Outer loop keeps trying to find tasks in case of single thread
	...
	if (victim_tid != -1) { // found last victim
	  ...
	  do { // Find a different thread to steal work from.
	    ...
	    //TODO:: Check if victim has tasks in its queue.
	    //if (other_thread.td.td_deque_tail - other_thread.td.td_deque_head == 0) continue;
	    asleep = 0;
...
#ifdef KMP_USE_XQUEUE
      if (!use_own_tasks && TCR_4(threads_data[tid].td.td_task_q[0]->td_deque[threads_data[tid].td.td_task_q[0]->td_deque_head]) != NULL)
#else
    ...
    if (nthreads == 1)
      ...
      /*#ifdef KMP_USE_XQUEUE
      //Hack to terminate any threads waiting to steal
      threads_data[__kmp_tid_from_gtid(gtid)].td.round++;
      //threads_data[__kmp_tid_from_gtid(gtid)].td.steal_req_id = UINT_MAX;
      #endif*/
      return FALSE;
    }
  }
...
static void __kmp_alloc_task_q(kmp_info_t *thread,
  ...
  KMP_DEBUG_ASSERT(thread_data->td.td_task_q == NULL);
  ...
  for (int queue_id = 0; queue_id < thread_data->td.num_queues; queue_id++) {
    thread_data->td.td_task_q[queue_id] = (kmp_taskq_t *) __kmp_allocate(sizeof(kmp_taskq_t));
    thread_data->td.td_task_q[queue_id]->td_deque = (volatile kmp_taskdata_t **)__kmp_allocate(
											       INITIAL_TASK_DEQUE_SIZE * sizeof(kmp_taskdata_t *));
    thread_data->td.td_deque_size = INITIAL_TASK_DEQUE_SIZE;
    KMP_DEBUG_ASSERT(thread_data->td.td_task_q[queue_id]->td_deque_head == 0);
...
#ifdef KMP_USE_XQUEUE
  if (thread_data->td.td_task_q != NULL) {
    for(int qid = 0; qid < thread_data->td.num_queues; qid++) {
      __kmp_free(thread_data->td.td_task_q[qid]);
    }
    thread_data->td.td_task_q = NULL;
  }
#else
  if (thread_data->td.td_deque != NULL) {
    __kmp_acquire_bootstrap_lock(&thread_data->td.td_deque_lock);
...
#ifdef KMP_USE_XQUEUE
      if (thread_data->td.td_task_q == NULL)
	__kmp_alloc_task_q(team->t.t_threads[i], thread_data, task_team);
#ifdef NUMA_AWARE      
      if (task_team->tt.tt_nproc < task_team->tt.tt_num_cores_per_zone)
	thread_data->td.numa_done = true;
#endif
...
#ifdef KMP_USE_XQUEUE
  thread_data->td.td_task_q[0]->td_deque[thread_data->td.td_task_q[0]->td_deque_tail] = taskdata;
  thread_data->td.td_task_q[0]->td_deque_tail = (thread_data->td.td_task_q[0]->td_deque_tail + 1)
